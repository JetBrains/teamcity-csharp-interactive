...
public class CommandLineServiceTests
{
    ...
    public void ShouldRunProcess()
    {
        ...
        _processRunner.Setup(i => i.Run(startInfo.Object, Handler, stateProvider.Object, It.IsAny<IProcessMonitor>(), TimeSpan.FromSeconds(1))).Returns(33);
    ...
    public async Task ShouldRunProcessAsync()
    {
        ...
        var stateProvider = process.As<IProcessStateProvider>();
        var cmdService = CreateInstance();
        _processRunner.Setup(i => i.RunAsync(startInfo.Object, Handler, stateProvider.Object, It.IsAny<IProcessMonitor>(), token)).Returns(Task.FromResult((int?)33));
